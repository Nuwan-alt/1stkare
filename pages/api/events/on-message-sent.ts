      ...
      if (user_rooms) {
        ...
        if (receivers && receivers.length > 0) {
          receivers.forEach(async (ur: any) => {
            const date1 = dayjs(ur.user.last_seen)
            const date2 = dayjs()
            var seconds = Math.abs(date1.diff(date2, 'seconds'))

            // var seconds2= date1.diff(date2, 'seconds')

            console.log('seconds: ', seconds)
            // console.log('seconds2: ', seconds2)

            //  var seconds =  getSecondsDiff(
            //     new Date(ur.user.last_seen),
            //      new Date())
