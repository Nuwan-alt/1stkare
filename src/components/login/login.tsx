...
function Login(props: LoginProps) {
  ...
  useEffect(() => {
    if (!userLoading && user && query_continue) {
      ...
      router.push(zipAndCity ? `/city/${zipAndCity.city.alt_id}` : '/')
    }
  }, [])
    ...
    (e: FormEvent) => {
      ...
      getAuth()
        ...
        .then(async () => {
          ...
          const full_name = sessionStorage.getItem('full_name') ?? 'User'
          if (auth.currentUser?.emailVerified) {
            ...
            // setError({ message: 'Email is not verified', name: 'Email' })
            logout()
