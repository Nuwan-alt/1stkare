...
export async function deactivatePPPaymentRecord(
  paypal_subscription_id: string
) {
  await hasuraServerRequest<
    { update_payments: { affected_rows: number } },
  ...
  >(
    gql`
      mutation DeactivatePPPaymentRecord($paypal_subscription_id: String) {
        update_payments(
          where: { paypal_subscription_id: { _eq: $paypal_subscription_id } }
          _set: { active: false }
        ) {
          affected_rows
          returning {
